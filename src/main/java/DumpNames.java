...
/**
 * Dumps the names of all USB devices by using the javax-usb API. On Linux this
 * can only work when your user has write permissions on all the USB device
 * files in /dev/bus/usb (Running this example as root will work). On Windows
 * this can only work for devices which have a libusb-compatible driver
